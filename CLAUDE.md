...
16. **A `repin` must name a cost**, and that cost must be an Economy Unit.
...
18d. **Restoration is its own analysis at R-1250**, on persistent state, after harm has fully landed. Not a Guard, not a vector in the harm Space, never cancels incoming harm, never reduced by one. A deliberate exception, and there will be others — this pipeline earns exceptions by worked cases, not by symmetry.
...
19. **Every Lens must be Calibrated** — the same distribution over magnitude the Resolution Socket produces. An uncalibrated Lens cannot ship.
