...
| **Aug 2026 · brand** | **The name is VECTUS — the system and the platform, one name, per Q5.4. Clearance not yet run** | Dylan's choice, delivered with a full identity board the same day. From *vector*, the word the design stands on. The knockout searches and intent-to-use filing are the next brand action and gate everything downstream |
| **Aug 2026 · brand** | **The Vectus identity board adopted as the base design principles** — documented in `brand-identity.md` | Tagline *Your Story. Your World. Your Way.*; essence *Focus on play. Freedom to create. Stories that move.*; the vector-figure mark; a neutral dark platform palette with **Dynamic Signal** — each world sets its own accent while the chrome stays constant; Aeonik/Inter type (Aeonik license required); and the **eight-step play-register vocabulary** (Intent → Direction → Force → Interaction → Layers → Resolution → Outcome → Your Story), which is the Q5.6 answer in shape: players meet the eight steps plus *vector, magnitude, chance*; the engine register never reaches a player screen. Flags awaiting Dylan: the VTTRPG badge vs the refuse-the-VTT-frame rule (recommended: own it as a coined category), and *solo play* as a new unscoped promise (Q4.5) |
| **Aug 2026 · brand** | **VTTRPG is owned as a coined category** | Dylan: *"we are a platform that is its own tabletop game in one."* Vectus does not host other games; the game is the platform. The category story is told that way everywhere, and "VTT" never appears unmodified |
| **Aug 2026 · Q4.5** | **Solo play is authored Adventure modules that run without a GM — pipelined before release, not immediately** | Content plays the GM: rails, pre-declared Thresholds, Deciders with defaults. Website, character creation and creature stat blocks come first |
| **Aug 2026 · brand** | **No bought fonts. Space Grotesk Bold stands in for display; the end state is a custom Vectus typeface, held in this repo** | Dylan's call, and the reasoning is the project's whole personality: the things that are ours live in the repo — the words, the code, and eventually the letterforms. Inter stays for body and UI |
| **Aug 2026 · build** | **The website blueprint authored — `docs/build/`, eleven documents, built to the standard that any Claude session can construct the site cold** | The whole site, not just the play surface: public explainer, auth, campaigns, authoring studio, admin. Three infrastructure calls made with Dylan: **~$25/month hosting** (Fly.io app + Neon Postgres + Resend + R2 backups), **the code in a new private `vectus` repo** (this docs repo stays public), **magic-link auth, no passwords anywhere**. The four pre-code specs the phase-map required are now written: RFC-8785 canonical JSON + SHA-256 state hash, the keyed counter PRNG with recorded rolls, the safe-integer policy, and the gapless-seq WS reconnect protocol |
| **Aug 2026 · build** | **One authoring pipeline, two doors** | GM studio and super-admin bulk-inject run the identical validate→publish pipeline — the expressibility check (L30 realized) with additive/structural misfit classification. Agent-drafted content is mechanics-and-data only, schema-enforced (`agent_prose_forbidden`), always human-approved. Users author instances, never types — the forms make types impossible, not forbidden |
